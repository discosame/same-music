...
class Voices(VoiceClient):
    ...
    async def search_playlist(self, prompt: str):
        ...
        with YoutubeDL(ydl_opts) as ydl:
            playlist_results = await asyncio.to_thread(
                ydl.extract_info, prompt, download=False
            )
            played = False
    ...
    async def search_title(self, prompt: str):
        ...
        with YoutubeDL(ydl_opts) as ydl:
            search_results = await asyncio.to_thread(
                ydl.extract_info, f"ytsearch:{prompt}", download=False
            )
    ...
    async def play(self):
        ...
        if self.looped:
            self.items.append(next_song_info)
        ...
        try:
            ...
            await self.bot.info[self.channel.id]["latest_message"].channel.send(
                embeds=[e], delete_after=15
            )
    ...
    async def pause(self):
        ...
        e.set_field_at(
            EmbedFIeld.status, name="ステータス", value=f"{self.bot.pause_emoji} 再生停止中"
        )
    ...
    async def resume(self):
        ...
        e = lastest_message.embeds[0]
        e.set_field_at(
            EmbedFIeld.status, name="ステータス", value=f"{self.bot.play_emoji} 再生中"
        )
    ...
    async def loop_enable(self):
        ...
        embed.set_field_at(
            EmbedFIeld.songs, name="追加済み曲一覧 (再生中の曲は除外されます)", value=values
        )
...
class Music(commands.Cog):
    ...
    async def on_send_log_next_song(self, voice_client: Voices, next_info):
        ...
        e.add_field(
            name="プレイリストについて",
            value="プレイリストの曲を全て取得するのは時間がかかるためスキップや削除をしたい場合は、反映されるまで暫くお待ちください",
            inline=False,
        )
        ...
        if cog_voice_client["edit"]:
            ...
            m = await cog_voice_client["latest_message"].channel.send(
                embeds=[e], view=view
            )
...
class SelectSongView(ui.View):
    def __init__(self, client: Voices):
        ...
        options = [
            SelectOption(
                label=items["video_title"], value=i, description=items["video_url"]
            )
            for i, items in enumerate(client.items)
        ]
...
class ADDSONGView(ui.View):
    ...
    @ui.button(
        row=1, emoji="🔼", label="曲追加", style=discord.ButtonStyle.green, custom_id="join"
    )
    async def add_song(self, interaction: discord.Interaction, _):
        ...
        e.add_field(name="追加した曲が反映されない？", value="Youtubeから曲を取得出来次第、次の曲再生時に反映されます")
    ...
    async def delete_song(self, interaction: discord.Interaction, _):
        ...
        if not (
            voice_client := interaction.client.info[voice.channel.id].get("client")
        ):
            e = Embed.error("まだ曲を追加して無いよ")
            return await interaction.response.send_message(embeds=[e])
        ...
        for value in view.values:
            deleted_song = interaction.client.info[voice.channel.id][
                "client"
            ].items.pop(int(value))
    ...
    async def update_song_list(self, interaction: discord.Interaction, _):
        ...
        e.set_field_at(
            EmbedFIeld.songs, name=e.fields[EmbedFIeld.songs].name, value=value
        )
